...
const keyButtons = Array.from(document.getElementsByClassName("keyboard-button"));
...
function colorKeyButton(letter, color) {
    for (const element of document.getElementsByClassName('keyboard-button')) {
        console.log(`Letter: ${letter}, Button Text: ${element.textContent}, Current Color: ${element.style.backgroundColor}, New Color: ${color}`);
        if (element.textContent.toLowerCase() === letter) {
            if (element.style.backgroundColor === 'forestgreen') {
                return;
            }
            if (element.style.backgroundColor === 'mediumvioletred' && color !== 'forestgreen') {
                return;
            }
            if (color === '#508d9c') {
            ...
            console.log(`Before setting color - Current Color: ${element.style.backgroundColor}, New Color: ${color}`);
            element.style.backgroundColor = color;
            console.log(`After setting color - Current Color: ${element.style.backgroundColor}, New Color: ${color}`);
            break;
        }
    }
}
...
function updateWordDisplay(rowIndex) {
    ...
    for (let i = 0; i < secretWord.length; i++) {
        ...
        const letterInstancesSecret = getLetterInstances(box.textContent, secretWord);
        const letterInstancesGuess = getLetterInstances(box.textContent, guessedWord);
        const letterPosition = getLetterPosition(box.textContent, guessedWord, i);
            ...
            if (letterInstancesGuess > letterInstancesSecret && letterPosition > letterInstancesSecret) {
                //if(guessedWord[i] === secretWord[i]){     //Case of word with 2 letters. first in wrong position (purple), second in correct position (stays blue).
